...
from mmseg_custom.models.segmentors import HRDAEncoderDecoder
from mmseg.models import SEGMENTORS, build_segmentor
from mmseg_custom.models.uda.fcdiscriminator import FCDiscriminator
from mmseg_custom.models.uda.uda_decorator import UDADecorator
from mmseg_custom.models.utils.dacs_transforms import denorm, get_mean_std
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        for k in pred_trg.keys():
            ...
            loss_G = self.loss_fn_D(
                ...
                Variable(
                    torch.FloatTensor(
                        D_out.data.size()).fill_(source_label)).to(D_out.device))  #.cuda()
        ...
        for k in pred.keys():
            ...
            loss_D = self.loss_fn_D(
                ...
                Variable(
                    torch.FloatTensor(
                        D_out_src.data.size()).fill_(source_label)).to(D_out_src.device))  #.cuda()
        ...
        for k in pred_trg.keys():
            ...
            loss_D = self.loss_fn_D(
                ...
                Variable(
                    torch.FloatTensor(
                        D_out_trg.data.size()).fill_(target_label)).to(D_out_trg.device))   #.cuda()
