    ...
    def forward(self, features, labels=None, mask=None, features_2=None, labels_2=None, reliability=None):
        ...
        if len(features_2.shape) < 4:
            raise ValueError('`features` needs to be [bsz, n_samples, n_views, ...],'
                             'at least 4 dimensions are required')
        if len(features.shape) > 4:
            features = features.view(features.shape[0], features.shape[1], features.shape[2], -1)   #features:(b, w*h, 1, feat_dim)
        if len(features_2.shape) > 4:
            features_2 = features_2.view(features_2.shape[0], features_2.shape[1], features_2.shape[2], -1)    #features_2:(b, w*h, 1, feat_dim)
        ...
        if labels is not None and mask is not None:
            ...
            if labels_2.shape[1] != n_samples:
                raise ValueError('Num of labels does not match num of features')
            # [bsz, bsz]
            reliability_mask = reliability.unsqueeze(1).repeat(1, reliability.shape[1], 1)  #(b, w*h, w*h)
            mask = torch.eq(labels, labels_2.transpose(1,2)).float().to(device)             #(b, w*h, w*h)
        else:
            # [bsz, bsz]
        ...
        contrast_feature = torch.cat(torch.unbind(features_2, dim=2), dim=1)    #contrast_feature:(b, w*h, feat_dim)
        if self.contrast_mode == 'one':
            anchor_feature = features[:, :, 0]    #anchor_feature:(b, w*h, feat_dim)
            anchor_count = 1
        ...
        anchor_dot_contrast = torch.div(
            ...
            self.temperature)                    #anchor_dot_contrast:(b, w*h, w*h)
        # for numerical stability
        logits_max, _ = torch.max(anchor_dot_contrast, dim=2, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()

        # tile mask
        mask = mask.repeat(1, anchor_count, contrast_count)    #(b, w*h, w*h)
