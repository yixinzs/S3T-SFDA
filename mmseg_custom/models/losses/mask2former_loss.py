...
from mmseg_custom.utils.point_features import point_sample, get_uncertain_point_coords_with_randomness
from mmseg_custom.utils.misc import is_dist_avail_and_initialized, nested_tensor_from_tensor_list, get_world_size
from mmseg_custom.utils.matcher import HungarianMatcher
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, class_weight=2.0, mask_weight=5.0, dice_weight=5.0, eos_coef=0.1, losses=None,
                 num_points=12544, oversample_ratio=3.0, importance_sample_ratio=0.75, loss_name='loss_mask2former'):
        ...
        self.matcher = HungarianMatcher(cost_class=class_weight, cost_mask=mask_weight, cost_dice=dice_weight, num_points=num_points)
        self.weight_dict = {"loss_ce": class_weight, "loss_mask": mask_weight, "loss_dice": dice_weight}
        self.eos_coef = eos_coef
        self.losses = losses
        self.device = torch.device("cuda", 0)
        empty_weight = torch.ones(self.num_classes + 1).to(self.device)
    ...
    def forward(self, outputs, gt_masks,
        ...
        """
        gt_masks = gt_masks.squeeze(dim=1)
        # print('---------------------gt_masks:{}'.format(gt_masks.shape))
        ...
        # print('-----------------------------pred_logits:{},pred_masks:{}, aux_outputs:{}'.format(outputs['pred_logits'].shape, outputs['pred_masks'].shape, len(outputs["aux_outputs"])))
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                # print('---------------------aux_outputs:{}'.format(aux_outputs["pred_logits"].shape))
    ...
    def _get_targets(self, gt_masks):
        ...
        for mask in gt_masks:
            ...
            # print('-----------------------------masks:{},labels:{}'.format(binary_masks.shape, labels.shape))
        return targets
