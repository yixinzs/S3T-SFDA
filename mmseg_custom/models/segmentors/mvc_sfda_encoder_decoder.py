...
class MVCSFDAEncoderDecoder(BaseSegmentor):
    ...
    def __init__(self, **cfg):
        ...
        self.his_model.eval()
        self.proj_head = ProjectionHead(dim_in=self.feat_channels, proj_dim=256)   # daformer_mitb2:64 + 128 + 320 + 512   #convnext_small:96 + 192 + 384 + 768
    ...
    def forward_train(self,
        ...
        if self.model.with_auxiliary_head:
            target_aux_head = self.model.auxiliary_head.forward(x)
            target_aux_pred = resize(input=target_aux_head, size=img.shape[2:], mode='bilinear', align_corners=self.model.align_corners)  # TODO:rois_imgs  img
        ...
        if self.model.with_auxiliary_head:
            st_aux_loss = self.criterion(target_aux_pred, psd_label.long()) * 0.4
            pesudo_p_aux_loss = (st_aux_loss * exp_variance).mean()
            target_losses['pesudo_pos_aux.loss'] = pesudo_p_aux_loss
