...
def downscale_label_ratio(gt,
    ...
    out = F.one_hot(
        out.squeeze(1), num_classes=n_classes + 1).permute(0, 3, 1, 2)   #BxCxHxW  C=n_classes + 1
    assert list(out.shape) == [bs, n_classes + 1, orig_h, orig_w], out.shape
    out = F.avg_pool2d(out.float(), kernel_size=scale_factor)   #F.avg_pool2d https://blog.csdn.net/qq_38417994/article/details/114290934
