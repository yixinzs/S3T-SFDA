...
class SegmentationMetric(object):
    ...
    def update(self, preds, labels):
        ...
        def evaluate_worker(self, pred, label):
            ...
            # correct_class, label_class = pixel_accuracy_class(pred, label, self.nclass)
            ...
            if self.distributed:
                ...
                # correct_class = reduce_tensor(correct_class.cuda())
                # label_class = reduce_tensor(label_class.cuda())
            ...
            # self.total_correct += correct.item()   #correct.item()
            # self.total_label += labeled.item()    #labeled.item()

            # if self.total_inter.device != inter.device:
            #     self.total_inter = self.total_inter.to(inter.device)
            #     self.total_union = self.total_union.to(union.device)
            # if self.total_correct_class.device != correct_class.device:
            #     self.total_correct_class = self.total_correct_class.to(correct_class.device)
            #     self.total_label_class = self.total_label_class.to(label_class.device)
            # self.total_inter += inter
            # self.total_union += union
            # self.total_correct_class += correct_class
            # self.total_label_class += label_class
    ...
    def get(self, return_category_iou=False):
        ...
        # pixAcc_class = 1.0 * self.total_correct_class / (2.220446049250313e-16 + self.total_label_class)
        ...
        # scores = {'pixAcc':pixAcc, 'precision':precision_class, 'recall': recall_class,
        #           'f1':f1_class, 'mIoU': mIoU, 'FWIoU': FWIoU}
        scores = [pixAcc, precision_class, recall_class, f1_class, mIoU, FWIoU]
        return self.confusion_matrix #scores
    ...
    def reset(self):
        ...
        # self.total_label = 0
        # self.total_correct_class = 0
        # self.total_label_class = 0
        self.confusion_matrix = torch.zeros((self.nclass, self.nclass)).cuda() #torch.zeros((self.nclass, self.nclass)).cuda()
